...
impl SearchService {
    ...
    pub fn invalidate(&self, paths: &[PathBuf]) {
        ...
        for root in index.roots.values_mut() {
...
fn search(index: &Mutex<SearchIndex>, criteria: SearchCriteria) -> ServiceResult<SearchResult> {
    ...
    for root in paths {
        ...
        let rebuild = index.roots.get(&key).is_none_or(|root| root.invalidated);
        if rebuild {
            index
                .roots
                .insert(key.clone(), build_index(&root, criteria.recursive));
            reused_index = false;
        }
        let root_index = index
            .roots
            .get_mut(&key)
            .expect("search index was just created or already existed");
        if content_query.is_some() && !root_index.content_ready {
            for indexed in &mut root_index.entries {
                if indexed.entry.is_dir || indexed.entry.size > MAX_CONTENT_BYTES {
                    continue;
                }
                indexed.content = fs::read_to_string(&indexed.entry.path)
                    .ok()
                    .map(|content| content.to_lowercase());
                content_reads += 1;
            }
            root_index.content_ready = true;
            reused_index = false;
        }
        indexed_entries += root_index.entries.len();

        for indexed in &root_index.entries {
            if !matches_criteria(
                indexed,
                &criteria,
                name_matcher.as_ref(),
...
fn build_index(root: &Path, recursive: bool) -> RootIndex {
...
mod tests {
    ...
    fn content_is_indexed_once_and_invalidated_after_a_watcher_change() {
        ...
        assert_eq!(miss.content_reads, 0);
    ...
    fn ten_thousand_entry_index_query_performs_zero_file_reads() {
        ...
        let mut entries: Vec<_> = (0..10_000)
            .map(|index| IndexedEntry {
                ...
                content: (index < 3).then(|| "needle".to_string()),
            })
            .collect();
        entries.push(IndexedEntry {
            entry: explorie_core::FileEntry {
