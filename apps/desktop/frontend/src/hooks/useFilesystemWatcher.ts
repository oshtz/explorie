...
import { loadRemoteDrives, type RemoteDriveStatus } from '../utils/remoteDrives';
...
export function isRemoteDrivePath(
  path: string,
  profiles: readonly { mountTarget: string }[],
  statuses: readonly { mountPath?: string | null }[] = []
): boolean {
  if (profiles.some((profile) => isPathCoveredByMount(path, profile.mountTarget))) {
    return true;
  }
  return statuses.some((status) => isPathCoveredByMount(path, status.mountPath));
}
...
export function useFilesystemWatcher({
  ...
  useEffect(() => {
    ...
    const coversVisiblePath = (mountPath: string | null | undefined) =>
      normalizedPathsRef.current.some((path) => isPathCoveredByMount(path, mountPath));

    const handleRemoteStatus = (status: RemoteDriveStatus) => {
      if (disposed || !coversVisiblePath(status.mountPath)) return;
      markUnavailable();
    };
    ...
    const startWatching = async () => {
      ...
      const profiles = loadRemoteDrives();
      if (watchPaths.some((path) => isRemoteDrivePath(path, profiles))) {
        markUnavailable();
        return;
      }
